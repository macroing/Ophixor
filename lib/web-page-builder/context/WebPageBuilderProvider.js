...
import { WebPageBuilderActionsContext, WebPageBuilderDataContext, WebPageBuilderEditorRendererContext, WebPageBuilderPageSchemaContext, WebPageBuilderPageStateContext, WebPageBuilderRendererContext, WebPageBuilderRuntimeContext, WebPageBuilderUIContext } from "./WebPageBuilderContext";
...
export function WebPageBuilderProvider({ children, initialComponentTemplates = {}, initialNow = Date.now(), initialPage, isShowingContentOnly, models = {}, onPageChange, pageData, pageSchema, resolveUrl, setPageData, setState, state }) {
  ...
  useEffect(() => {
    if (componentIndexRebuildingRule) {
      ...
      const component = componentIndexRebuildingRule.component;
      ...
      if (type === "addComponent") {
        ...
        if (component) {
          findAllIds(component, ids);
        } else {
          ids.push(componentId);
        }

        setComponentIndex((oldComponentIndex) => buildComponentIndexForMany(components, ids, { ...oldComponentIndex }, history.page).index);
      } else if (type === "redo") {
        setComponentIndex((oldComponentIndex) => buildComponentIndex(components, { ...oldComponentIndex }, history.page));
  ...
  }, [componentIndex, history, setComponentIndexRebuildingRule]);
  ...
  const editorRendererValue = useMemo(
    () => ({
      dragState,
      selectedId,
    }),
          ...
          <WebPageBuilderDataContext.Provider value={data}>
            <WebPageBuilderUIContext.Provider value={ui}>
              <WebPageBuilderPageStateContext.Provider value={pageState}>
                <WebPageBuilderRendererContext value={rendererValue}>
                  <WebPageBuilderEditorRendererContext value={editorRendererValue}>{children}</WebPageBuilderEditorRendererContext>
                </WebPageBuilderRendererContext>
...
function findAllIds(component, ids = []) {
  if (component && typeof component === "object" && !Array.isArray(component) && component.id) {
    ...
    if (component.slots && typeof component.slots === "object" && !Array.isArray(component.slots)) {
      for (const slotName in component.slots) {
        if (component.slots.hasOwnProperty(slotName)) {
          const slot = component.slots[slotName];

          if (Array.isArray(slot)) {
            for (let i = 0; i < slot.length; i++) {
              findAllIds(slot[i], ids);
            }
          }
        }
      }
