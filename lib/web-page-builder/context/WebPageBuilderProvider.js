...
export function WebPageBuilderProvider({ children, initialComponentTemplates = {}, initialNow = Date.now(), initialPage, isShowingContentOnly, models = {}, onPageChange, pageData, pageSchema, resolveUrl, setPageData, setState, state }) {
  ...
  const dataStoreRef = useRef(createDataStore(models, history.page, pageData, pageSchema, relationIndexes, resolveUrl, setIntegrationDataMap));
...
function createDataStore(models, page, pageData, pageSchema, relationIndexes, resolveUrl, setIntegrationDataMap) {
  const context = {
    models,
    page,
    pageData,
