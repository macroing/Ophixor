...
export default function TableData({ children, componentId, editor, styles = importedStyles, theme, ...styleProps }) {
  ...
  const themeClass = typeof theme === "string" && THEMES.has(theme) ? styles[`table_data_${theme}`] : undefined;

  const isEmpty = !children || (Array.isArray(children) && children.length === 0);

  return (
    <td className={classNames(styles.table_data, isEmpty && styles.table_data_empty, ...editorClasses, themeClass)} data-pc-id={componentId} style={style} {...editorProps}>
      {!isEmpty && children}
