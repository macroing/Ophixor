  ...
  function renderControlsForParameter(key, parameter) {
    return (
      <>
        ...
        {node.type !== "literal" && !parameter.isExpressionAllowed && (parameter.type === "string" || (Array.isArray(parameter.type) && parameter.type.includes("string"))) && <DarkInput id={key} isDebounceDisabled={true} onChange={(e) => updateParameterValue(key, e.target.value)} value={data[key] ?? ""} />}
...
function LiteralNodeEditor(props) {
  ...
  return (
    <>
      ...
      {type === "array" && <DarkTextArea isDebounceDisabled={true} color={Array.isArray(array) ? "#22c55e" : "#ef4444"} onChange={onChangeArray} rows={5} value={value} />}
      {type === "boolean" && <DarkSwitch checked={!!value} id={dataKey} onChange={onChangeBoolean} />}
      {type === "number" && <DarkInput isDebounceDisabled={true} onChange={onChangeNumber} type="number" value={value} />}
      {type === "object" && <DarkTextArea isDebounceDisabled={true} color={object && typeof object === "object" ? "#22c55e" : "#ef4444"} onChange={onChangeObject} rows={5} value={value} />}
      {type === "string" && <DarkTextArea isDebounceDisabled={true} onChange={onChangeString} rows={5} value={value} />}
    </>
  );
}
