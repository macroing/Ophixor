...
export default function Input(props) {
  ...
  const themeClass = typeof theme === "string" && THEMES.has(theme) ? styles[`input_${theme}`] : undefined;
  ...
  return <input className={cx(styles.input, editor?.isSelected && styles.input_selected, (!editor || editor?.isShowingContentOnly) && styles.input_content_only, themeClass)} data-pc-id={componentId} disabled={safeDisabled} draggable={editor?.draggable} id={id ? id : undefined} name={safeName} onBlur={onBlur} onChange={onChangeImpl} onContextMenu={editor?.onContextMenu} onDragStart={editor?.onDragStart} onMouseDown={editor?.onMouseDown} placeholder={placeholder} readOnly={safeReadOnly ? safeReadOnly : !editor || editor?.isShowingContentOnly ? false : true} spellCheck={safeSpellCheck} style={style} type={type} value={internalValue} />;
}
