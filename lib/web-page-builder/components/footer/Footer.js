        ...
        <div className={styles.footer_grid} style={{ gridTemplateColumns: isMobile || isTablet ? "repeat(auto-fit, minmax(250px, 1fr))" : "repeat(auto-fit, minmax(200px, 1fr))", justifyItems: isMobile ? "center" : "legacy", textAlign: isMobile ? "center" : "left" }}>
          ...
          {safeItems.map((item, itemIndex) => (
            <div className={styles.footer_links} key={"item-" + itemIndex}>
              <h4>
                {item?.href ? (
                  <Link href={typeof resolveUrl === "function" ? resolveUrl(item.href) : item.href} rel={item.target === "_blank" ? "noopener noreferrer" : undefined} target={item.target}>
                    {item?.label || ""}
                  </Link>
                ) : (
                  item?.label || ""
              ...
              {Array.isArray(item?.items) && item.items.length > 0 && (
                <ul>
                  {item.items.map((innerItem, innerItemIndex) => (
                    <li key={"item-" + itemIndex + "-" + innerItemIndex}>
                      {innerItem?.href ? (
                        <Link href={typeof resolveUrl === "function" ? resolveUrl(innerItem.href) : innerItem.href} rel={innerItem.target === "_blank" ? "noopener noreferrer" : undefined} target={innerItem.target}>
                          {innerItem?.label || ""}
                        </Link>
                      ) : (
                        innerItem?.label || ""
