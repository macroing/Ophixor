...
export default function TableHeader({ children, componentId, editor, styles = importedStyles, theme, ...styleProps }) {
  ...
  const themeClass = typeof theme === "string" && THEMES.has(theme) ? styles[`table_header_${theme}`] : undefined;

  const isEmpty = !children || (Array.isArray(children) && children.length === 0);

  return (
    <th className={classNames(styles.table_header, isEmpty && styles.table_header_empty, ...editorClasses, themeClass)} data-pc-id={componentId} style={style} {...editorProps}>
      {!isEmpty && children}
