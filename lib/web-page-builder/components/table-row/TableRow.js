...
export default function TableRow({ children, componentId, editor, styles = importedStyles, theme, ...styleProps }) {
  ...
  const themeClass = typeof theme === "string" && THEMES.has(theme) ? styles[`table_row_${theme}`] : undefined;

  const isEmpty = !children || (Array.isArray(children) && children.length === 0);

  return (
    <tr className={classNames(styles.table_row, ...editorClasses, themeClass)} data-pc-id={componentId} style={style} {...editorProps}>
      {!isEmpty && children}
