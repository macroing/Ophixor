...
export default function Select(props) {
  ...
  return (
    <select className={cx(styles.select, editor?.isSelected && styles.select_selected, (!editor || editor?.isShowingContentOnly) && styles.select_content_only, themeClass)} data-pc-id={componentId} disabled={safeDisabled} draggable={editor?.draggable} id={id ? id : undefined} name={safeName} onChange={onChange} onContextMenu={editor?.onContextMenu} onDragStart={editor?.onDragStart} onMouseDown={editor?.onMouseDown} style={style} value={value}>
      {options.map((option, optionIndex) => (
