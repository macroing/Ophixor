...
export default function PixabayMediaSelector(props) {
  ...
  function handleSelect(image) {
    setSelectedId(image.id.toString());
  }
    ...
    <Section padding="0px">
      {isSearching ? (
        ...
        <Grid gridTemplateColumns={isMobileOriginal ? "1fr" : isTabletOriginal ? "repeat(2, 1fr)" : "repeat(3, 1fr)"} padding="0px">
          {images.map((image) => (
            <DarkCard alignItemsBody="center" borderColor={selectedId === image.id.toString() ? "#2563eb" : "#374151"} borderColorHover={selectedId === image.id.toString() ? "#2563eb" : "#1e3a8a"} boxShadowHover="0 16px 48px rgba(15, 23, 42, 0.18)" cursor="pointer" isHoverEffect={true} key={image.id} onClick={() => handleSelect(image)} paddingBody="0.75rem" transformHover="translateY(-4px)" transition="all 0.3s ease">
              {{
                slots: {
                  header: [],
                  body: [<Image alt={image.tags} borderColor="#2d3748" borderWidth="1px" cursor="pointer" height="150px" key="1" objectFit="contain" src={image.webformatURL} width="100%" />, <Text color="#e5e7eb" cursor="pointer" key="2" style={{ fontSize: "0.8rem", textAlign: "center" }} text={image.tags} />],
                  footer: [],
                },
              }}
            </DarkCard>
