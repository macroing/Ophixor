...
export default function PixabayMediaSelector(props) {
  ...
  async function loadImagesOnPage(pageToLoad) {
    try {
      ...
      setError(platform.websiteAdmin.media.pixabayMediaSelector.error[language]);
    } finally {
    ...
    <Section padding="0px">
      {isSearching || isSearchingNewPage ? (
        <Section backgroundColor="rgba(96, 165, 250, 0.1)" borderColor="#1e3a8a" borderRadius="4px" borderWidth="1px" padding="0.5rem 1rem">
          <Text color="#60a5fa" text={platform.websiteAdmin.media.pixabayMediaSelector.loading[language]} />
        </Section>
      ) : error ? (
        ...
        <>
          ...
          {pagination.length > 0 && (
            <div className={styles.pagination}>
              ...
              >
                <Icon icon={faChevronDoubleLeft} size={10} />
              </div>
              ...
              >
                <Icon icon={faChevronDoubleRight} size={10} />
              </div>
            </div>
          )}
        </>
