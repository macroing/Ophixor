...
export default function ComponentWrapper({ children, component, index, parentId, slotIndex, slotName }) {
  ...
  return (
    <EditorComponentWrapper component={component} index={index} parentId={parentId} slotIndex={slotIndex} slotName={slotName}>
      {children}
    </EditorComponentWrapper>
  );
}

function EditorComponentWrapper({ children, component, index, parentId, slotIndex, slotName }) {
  ...
  const onDragStart = useCallback(
    ...
    [componentId, editorStore, index, parentId, slotIndex, slotName, type],
  );
  ...
  const slot = useCallback(
    (slotName, componentFactory, slotIndex = null) => {
      hasExplicitSlotsRef.current = true;

      return (
        <SlotWrapper component={component} key={componentId + "-" + slotName + (typeof slotIndex === "number" ? "-" + slotIndex : "")} parentId={componentId} slotId={`${componentId}-${slotName}`} slotIndex={slotIndex} slotName={slotName} slotOwnerType={component.type}>
          {componentFactory}
        </SlotWrapper>
      );
    },
