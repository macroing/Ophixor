...
export default function BindingWrapper(props) {
  ...
  return (
    <div className={styles.binding_wrapper}>
      ...
      {isExpression && canUseExpression && !property?.isExpressionUnsupported && <DarkButton onClick={(e) => setIsOpen(true)}>{platform.websiteAdmin.pages.editor.changeExpression[language]}</DarkButton>}
      {isExpression && canUseExpression && !property?.isExpressionUnsupported && <ExpressionEditorDialog componentType={componentType} dataScope={dataScope} expectedType={getExpectedType()} expression={safeValue.expression} isOpen={isOpen} isPlatformAdmin={isPlatformAdmin} onChange={(expr) => onChange({ ...safeValue, expression: expr })} onClose={onClose} plan={plan} />}
    </div>
  );
}
