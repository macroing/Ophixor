...
export default function MediaPicker(props) {
  ...
  function handleUploadComplete(isSuccessful) {
    if (isSuccessful) {
      setUploadMessage(platform.websiteAdmin.media.mediaPicker.uploadMessageSuccess[language]);
      ...
      setUploadMessage(platform.websiteAdmin.media.mediaPicker.uploadMessageFailure[language]);
  ...
  return (
    <DarkDialog dialogRef={dialogRef} height="100%" heightBody="100%" maxHeight="90vh" maxWidth="1200px" width="100%" widthBody="100%">
      {{
        slots: {
          header: [<Heading color="#e5e7eb" key="1" level="5" text={platform.websiteAdmin.media.mediaPicker.title[language]} />],
          body: [
            <div key="1" style={{ display: "grid", gap: "1rem" }}>
              {showUploader && canCreateMedia ? (
                <div style={{ alignItems: "center", display: "flex", gap: "1rem", justifyContent: uploadMessage && !isSelectingFromPixabay ? "space-between" : "flex-end" }}>
                  ...
                  <div style={{ alignItems: "center", display: "flex", gap: "1rem", justifyContent: isSelectingFromPixabay ? "flex-start" : "flex-end", width: isSelectingFromPixabay ? "100%" : "auto" }}>
                    {isSelectingFromPixabay && <DarkInput isDebounceDisabled={false} onChange={(e) => setSearchTerm(e.target.value)} placeholder={platform.websiteAdmin.media.mediaPicker.searchPlaceholder[language]} value={searchTerm} />}
                    {isSelectingFromPixabay && (
                      <DarkButton disabled={isSearching || searchTerm.length <= 3} onClick={() => setIsSearching(true)} theme="primary" type="button">
                        <Icon icon={faSearch} size={16} /> {platform.websiteAdmin.media.mediaPicker.search[language]}
                      </DarkButton>
                    )}
                    {!isSelectingFromPixabay && (
                      <DarkButton onClick={() => setIsSelectingFromPixabay(true)} type="button">
                        <Icon icon={faImage} size={16} /> {platform.websiteAdmin.media.mediaPicker.selectFromPixabay[language]}
                      </DarkButton>
                    )}
                    <DarkButton
                      onClick={() => {
                      ...
                      type="button"
                    >
                      <Icon icon={isSelectingFromPixabay ? faArrowLeft : faBrowser} size={16} /> {isSelectingFromPixabay ? platform.websiteAdmin.media.mediaPicker.back[language] : platform.websiteAdmin.media.mediaPicker.browse[language]}
                    </DarkButton>
                  </div>
                </div>
              ) : (
                <div style={{ alignItems: "center", display: "flex", gap: "1rem" }}>
                  <DarkInput isDebounceDisabled={true} onChange={(e) => setSearch(e.target.value)} placeholder={platform.websiteAdmin.media.mediaPicker.searchPlaceholder[language]} value={search} />
                  <DarkButton disabled={!canCreateMedia} onClick={() => setShowUploader(true)} theme="primary" type="button">
                    <Icon icon={faUpload} size={16} /> {platform.websiteAdmin.media.mediaPicker.upload[language]}
                  </DarkButton>
                </div>
              )}
              {showUploader && canCreateMedia && !isSelectingFromPixabay ? (
                ...
                <Text color="#e5e7eb" text={platform.websiteAdmin.media.mediaPicker.loading[language]} />
              ) : (
          ...
          footer: [
            <DarkButton key="1" onClick={onClickClose} type="button">
              <Icon icon={faCancel} size={16} /> {platform.websiteAdmin.media.mediaPicker.cancel[language]}
            </DarkButton>,
            <DarkButton disabled={!selectedId} key="2" onClick={handleConfirm} theme="primary" type="button">
              <Icon icon={faCheck} size={16} /> {platform.websiteAdmin.media.mediaPicker.select[language]}
            </DarkButton>,
          ],
        },
      }}
