...
export default function UnitInput(props) {
    ...
    if (!isMulti) {
      ...
      return (
        <div className={styles.inline_group}>
          <DarkInput isDebounceDisabled={true} onChange={(e) => onChange(e.target.value + unit)} type="number" value={number} />
        ...
        {parsed.map((v, i) => {
          ...
          return (
            <div className={styles.side} key={i}>
              <span>{platform.websiteAdmin.pages.editor[labels[i]][language]}</span>
              <DarkInput
  ...
  return (
    <div className={styles.unit_input + (mode === "fixed" ? " " + styles.unit_input_fixed : "")}>
      ...
      {mode === "function" && <DarkInput isDebounceDisabled={true} onChange={(e) => onChange(e.target.value)} value={value} />}
      {mode === "global" && <DarkSelect onChange={(e) => onChange(e.target.value)} options={globals.map((g) => ({ label: g, value: g }))} value={value} />}
      {mode === "raw" && <DarkInput isDebounceDisabled={true} onChange={(e) => onChange(e.target.value)} placeholder={platform.websiteAdmin.pages.editor.enterCSSValue[language]} value={value} />}
    </div>
  );
}
