...
export default function ImageUploader(props) {
  const formDataConfigurator = props.formDataConfigurator;
  ...
  const resetState = useCallback(() => {
    ...
    setFilter("none");
    setRotation(0);
  }, [setAspect, setCropActive, setFilter, setRotation]);
  ...
  const loadImage = useCallback(
    ...
    [resetState, setOriginalImage, setOriginalImageName, setWorkingImage],
  );
  ...
  async function upload() {
    ...
    exportCanvas.toBlob(async (blob) => {
      ...
      if (formDataConfigurator) {
        formDataConfigurator(formData);
      }
  ...
  useEffect(() => {
    if (selectedImagePixabay) {
      const currentSelectedImagePixabay = selectedImagePixabay;

      async function loadSelectedImagePixabay() {
        const file = await loadUrlIntoCanvas(currentSelectedImagePixabay.webformatURL, currentSelectedImagePixabay.user);
      ...
      loadSelectedImagePixabay();
    }
  }, [loadImage, selectedImagePixabay]);

  return (
    <div className={styles.wrapper}>
      <ControlsPanel applyCrop={applyCrop} cropActive={cropActive} filter={filter} onUpload={loadImage} originalImage={originalImage} restore={restore} rotation={rotation} setAspect={setAspect} setFilter={setFilter} setRotation={setRotation} styles={styles} toggleCrop={toggleCrop} upload={upload} width={width} />
...
function ControlsPanel({ applyCrop, cropActive, filter, onUpload, originalImage, restore, rotation, setAspect, setFilter, setRotation, styles, toggleCrop, upload, width }) {
  return (
    <DarkCard flexDirectionBody="column">
      {{
        slots: {
          header: [],
          body: [
            <div className={styles.row} key="1">
              <FileInputButtonLabel accept="image/*" onChange={(e) => onUpload(e.target.files[0])} theme={originalImage ? undefined : "primary"}>
                <Icon icon={faImage} size={16} /> Select Image
              </FileInputButtonLabel>
              <DarkSelect
                onChange={(e) => setFilter(e.target.value)}
                options={Object.entries(FILTERS).map(([key, filter]) => {
                  return { label: filter.label, value: key };
                })}
                value={filter}
              />
              <DarkSelect
            ...
            <div className={styles.row} key="2">
              <DarkButton disabled={!originalImage} onClick={toggleCrop} type="button">
                <Icon icon={faCrop} size={16} /> Crop
              </DarkButton>
              <DarkButton disabled={!originalImage || !cropActive} onClick={applyCrop} theme={originalImage && cropActive ? "primary" : undefined} type="button">
                <Icon icon={faCheck} size={16} /> Apply Crop
              </DarkButton>
              <DarkButton disabled={!originalImage} onClick={upload} theme={originalImage ? "primary" : undefined} type="button">
                <Icon icon={faUpload} size={16} /> Upload
              </DarkButton>
              <DarkButton disabled={!originalImage} onClick={restore} theme={originalImage ? "danger" : undefined} type="button">
                <Icon icon={faRefresh} size={16} /> Reset
              </DarkButton>
            </div>,
          ],
        },
...
async function loadUrlIntoCanvas(imageUrl, name) {
  try {
    ...
    const cleanFileName = name + ".jpg";

    const mockFileObject = new File([blob], cleanFileName, { type: blob.type || "image/jpeg" });
...
const FILTERS = {
  none: { label: "No filter", type: "none" },
