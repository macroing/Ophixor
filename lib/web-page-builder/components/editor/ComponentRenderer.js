...
import TabPaneItem from "../tab-pane-item/TabPaneItem";
...
const ComponentInstanceRenderer = memo(({ component, componentSchema, index, isEmpty = false, parentId, props, slotName }) => {
  ...
  return (
    <ComponentWrapper component={component} index={index} parentId={parentId} slotName={slotName}>
      {(editorProps) => (editorProps?.editor?.isShowingContentOnly && isEmpty ? null : <Component component={component} componentId={component.id} componentIndex={index} editor={editorProps?.editor} resolveUrl={resolveUrl} {...reactiveProps} {...slotProps} />)}
    </ComponentWrapper>
  );
});
