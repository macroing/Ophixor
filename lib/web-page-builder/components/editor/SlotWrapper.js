...
export default function SlotWrapper({ children, component, parentId, slotId, slotIndex, slotName, slotOwnerType }) {
  ...
  return (
    <EditorSlotWrapper component={component} parentId={parentId} slotId={slotId} slotIndex={slotIndex} slotName={slotName} slotOwnerType={slotOwnerType}>
      {children}
    </EditorSlotWrapper>
  );
}

function EditorSlotWrapper({ children, component, parentId, slotId, slotIndex, slotName, slotOwnerType }) {
  const { dragState } = useWebPageBuilderEditorRenderer();

  const { editorStore } = useWebPageBuilderRenderer();

  const internalSlotIdentifier = slotId + (typeof slotIndex === "number" ? "-" + slotIndex : "");
  ...
  const onDragLeave = useCallback(
    (e) => {
      ...
      if (dragDepthRef.current === 0) {
        if (editorStore.context.activeDropZoneRef.current === internalSlotIdentifier) {
    ...
    [dragDepthRef, editorStore, internalSlotIdentifier, setIsDraggingOver],
  );
  ...
  const onDragOver = useCallback(
    (e) => {
      ...
      if (editorStore.context.activeDropZoneRef.current !== internalSlotIdentifier) {
        editorStore.context.activeDropZoneRef.current = internalSlotIdentifier;

        setIsDraggingOver?.(true);
      }
    },
    [editorStore, internalSlotIdentifier, isDraggingOverAllowed, pointerRef, setIsDraggingOver],
  );
  ...
  const onDrop = useCallback(
        ...
        if (typeof dataString === "string") {
          ...
          if (data && isDraggingOverAllowed) {
            if (typeof data.action === "string" && data.action === "add-component" && typeof data.sourceType === "string") {
              editorStore.context.addComponent({ component: editorStore.context.createComponent(data.sourceType), parentId, slotIndex, slotName });
              ...
              editorStore.context.addComponentFromComponentTemplate({ parentId, slotIndex, slotName, type: data.type });
              ...
              editorStore.context.moveComponent({ sourceId: data.sourceId, sourceIndex: data.sourceIndex, sourceParentId: data.sourceParentId, sourceSlotIndex: data.sourceSlotIndex, sourceSlotName: data.sourceSlotName, targetIndex: Number.MAX_SAFE_INTEGER, targetParentId: parentId, targetSlotIndex: slotIndex, targetSlotName: slotName });
            }
          }
        }
    ...
    [componentId, dragDepthRef, editorStore, internalSlotIdentifier, isDraggingOverAllowed, parentId, setIsDraggingOver, slotIndex, slotName],
  );
  ...
  const object = useMemo(
    () => ({
      editor: {
        ...
        isNearDropTarget: editorStore.context.isShowingContentOnly ? undefined : !!dragState && editorStore.context.activeDropZoneRef.current !== internalSlotIdentifier && isNear(dropZoneContainerRef, pointerRef.current),
    ...
    [editorStore, dragState, dropZoneContainerRef, internalSlotIdentifier, isDraggingOver, isDraggingOverAllowed, isNear, onDragEnd, onDragEnter, onDragLeave, onDragOver, onDrop, pointerRef],
  );
