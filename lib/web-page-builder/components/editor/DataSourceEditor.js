...
export default function DataSourceEditor(props) {
  ...
  function createOptions() {
    ...
    newOptions.push({ label: platform.websiteAdmin.pages.editor.dataSourceEditor.none[language], value: "none" });

    if (canUseDataSourceModel) {
      newOptions.push({ label: platform.websiteAdmin.pages.editor.dataSourceEditor.model[language], value: "model" });
    }

    if (canUseDataSourceExpression) {
      newOptions.push({ label: platform.websiteAdmin.pages.editor.dataSourceEditor.expression[language], value: "expression" });
    }

    if (canUseDataSourceIntegration) {
      newOptions.push({ label: platform.websiteAdmin.pages.editor.dataSourceEditor.integration[language], value: "integration" });
    }
  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.column_2}>
        <DarkLabel htmlFor="source-type" text={platform.websiteAdmin.pages.editor.dataSourceEditor.source[language]} />
      ...
      {sourceType === "model" && canUseDataSourceModel && (
        <>
          <div className={styles.column_2}>
            <DarkLabel htmlFor="from" text={platform.websiteAdmin.pages.editor.dataSourceEditor.from[language]} />
            <DarkSelect onChange={(e) => updateQuery({ from: e.target.value })} options={[{ label: platform.websiteAdmin.pages.editor.dataSourceEditor.selectSource[language], value: "" }, ...fromOptions]} value={from} />
          </div>
          <div className={styles.section}>
            <DarkLabel text={platform.websiteAdmin.pages.editor.dataSourceEditor.filter[language]} />
            {filter.map((filter, filterIndex) => (
              <div className={styles.grid_2} key={filterIndex}>
                <DarkSelect
                  ...
                  options={[
                    { label: platform.websiteAdmin.pages.editor.dataSourceEditor.field[language], value: "" },
                    ...fields.map((field) => {
                ...
                />
                <DarkInput isDebounceDisabled={true} onChange={(e) => updateFilter(filterIndex, { value: e.target.value })} value={filter.value} />
          ...
          <div className={styles.section}>
            <DarkLabel text={platform.websiteAdmin.pages.editor.dataSourceEditor.sort[language]} />
            {sort.map((sortElement, sortElementIndex) => (
              <div className={styles.grid_2} key={sortElementIndex}>
                ...
                <DarkSelect
                  ...
                  options={[
                    { label: platform.websiteAdmin.pages.editor.dataSourceEditor.asc[language], value: "asc" },
                    { label: platform.websiteAdmin.pages.editor.dataSourceEditor.desc[language], value: "desc" },
                  ]}
          ...
          <div className={styles.column_2}>
            <DarkLabel htmlFor="limit" text={platform.websiteAdmin.pages.editor.dataSourceEditor.limit[language]} />
      ...
      {sourceType === "expression" && canUseDataSourceExpression && (
        <>
          <div className={styles.section}>
            <DarkLabel text={platform.websiteAdmin.pages.editor.dataSourceEditor.expression[language]} />
            <DarkButton onClick={(e) => setIsOpen(true)}>{platform.websiteAdmin.pages.editor.dataSourceEditor.changeExpression[language]}</DarkButton>
          ...
          <div className={styles.column_2}>
            <DarkLabel text={platform.websiteAdmin.pages.editor.dataSourceEditor.fallback[language]} />
            <DarkInput
      ...
      {sourceType === "integration" && canUseDataSourceIntegration && (
        <>
          <div className={styles.section}>
            <DarkLabel text={platform.websiteAdmin.pages.editor.dataSourceEditor.integration[language]} />
            <DarkButton onClick={() => setIsIntegrationPickerOpen(true)}>Select integration</DarkButton>
          </div>
          {dataSource?.integrationId && (
            <>
              <div className={styles.column_2}>
                <DarkLabel text={platform.websiteAdmin.pages.editor.dataSourceEditor.endpoint[language]} />
                <DarkSelect
                  onChange={(e) => {
              ...
              <div className={styles.section}>
                ...
                {Object.entries(dataSource.params || {}).map(([key, val]) => (
                  <div className={styles.grid_2} key={key}>
                    <span>{key}</span>
                    <DarkButton onClick={() => openParamExpression(key)}>{platform.websiteAdmin.pages.editor.dataSourceEditor.edit[language]}</DarkButton>
                  </div>
                ))}
              </div>
              <ExpressionEditorDialog
...
function collectSchemaFields(fields, prefix = "") {
  ...
  for (const [key, field] of Object.entries(fields)) {
    ...
    if (field.type === "single" && field.fields) {
      paths.push(...collectSchemaFields(field.fields, path));
    }
  }
