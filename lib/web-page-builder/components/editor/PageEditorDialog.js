...
import { faClose, faDesktop, faMobileAlt, faRedo, faSave, faTabletAlt, faUndo } from "@fortawesome/pro-solid-svg-icons";
...
export default function PageEditorDialog(props) {
  ...
  const { canRedo, canUndo, componentIndex, draftStart, isDraftEnabled, page, redo, setComponentIndexRebuildingRule, undo } = useWebPageBuilderPageState();
  ...
  function onClickRedo(e) {
    ...
    setComponentIndexRebuildingRule({ type: "redo" });
  }
  ...
  function onClickUndo(e) {
    ...
    setComponentIndexRebuildingRule({ type: "undo" });
  }
            ...
            {!isMobileOriginal && (
              <div className={styles.toggle_buttons}>
                <button className={styles.toggle_button + (name === "mobile" ? " " + styles.toggle_button_active : "")} onClick={onClickSimulateMobile} type="button">
                  <Icon icon={faMobileAlt} size={16} /> {platform.websiteAdmin.pages.editor.mobile[language]}
                </button>
                <button className={styles.toggle_button + (name === "tablet" ? " " + styles.toggle_button_active : "")} onClick={onClickSimulateTablet} type="button">
                  <Icon icon={faTabletAlt} size={16} /> {platform.websiteAdmin.pages.editor.tablet[language]}
                </button>
                <button className={styles.toggle_button + (name === "desktop" ? " " + styles.toggle_button_active : "")} onClick={onClickSimulateDesktop} type="button">
                  <Icon icon={faDesktop} size={16} /> {platform.websiteAdmin.pages.editor.desktop[language]}
                </button>
              </div>
            )}
