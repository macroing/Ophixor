...
export default function TextArea(props) {
  ...
  const themeClass = typeof theme === "string" && THEMES.has(theme) ? styles[`text_area_${theme}`] : undefined;
  ...
  return <textarea className={cx(styles.text_area, editor?.isSelected && styles.text_area_selected, (!editor || editor?.isShowingContentOnly) && styles.text_area_content_only, themeClass)} data-pc-id={componentId} disabled={safeDisabled} draggable={editor?.draggable} id={id ? id : undefined} name={safeName} onBlur={onBlur} onChange={onChangeImpl} onContextMenu={editor?.onContextMenu} onDragStart={editor?.onDragStart} onMouseDown={editor?.onMouseDown} placeholder={placeholder} readOnly={safeReadOnly ? safeReadOnly : !editor || editor?.isShowingContentOnly ? false : true} rows={rows ? rows : undefined} style={style} value={internalValue} />;
}
