      ...
      <Card alignItemsBody="flex-start" backgroundColor="var(--pc-semantic-status-danger-bg)" backgroundColorBody="transparent" backgroundColorBodyHover="transparent" backgroundColorHover="var(--pc-semantic-status-danger-bg)" borderColor="var(--pc-semantic-status-danger)" borderColorHover="var(--pc-semantic-status-danger)" element="form" flexGrow="0" gapBody="2rem" justifyContentBody="flex-start" onSubmit={onSubmit}>
        {{
          slots: {
            header: [],
            body: [
              <Heading color="var(--pc-semantic-status-danger-text)" key="1" level="3" text={platform.websiteAdmin.pages.deleteForm.title[language]} />,
              message && (
                <Alert key="2" theme="error">
                  <Text text={message} />
                </Alert>
              ),
              <Text color="var(--pc-semantic-status-danger-text)" key="3" text={platform.websiteAdmin.pages.deleteForm.text[language]} />,
              <Button borderRadius="8px" disabled={isShowingDialog} key="4" theme="danger">
                <Icon icon={faTrash} size={16} style={{ color: "inherit" }} /> {platform.websiteAdmin.pages.deleteForm.button[language]}
              </Button>,
            ],
            footer: [],
          },
            ...
            slots: {
              ...
              footer: [
                <Button key="1" onClick={onClickCancel}>
                  <Icon icon={faCancel} size={16} style={{ color: "inherit" }} /> {platform.websiteAdmin.pages.deleteForm.dialogCancel[language]}
                </Button>,
                <Button key="2" onClick={onClickDelete} theme="danger">
                  <Icon icon={faTrash} size={16} style={{ color: "inherit" }} /> {platform.websiteAdmin.pages.deleteForm.dialogDelete[language]}
                </Button>,
              ],
            },
          }}
